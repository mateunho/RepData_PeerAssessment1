...
Mean total number of steps taken per day is `r mean.daily.activity` and median is `r median.daily.activity`.
...
On average across all the days in the dataset, the 5-minute interval which contains the maximum number of steps is `r max.daily.activity`.
...
There are `r total.nas` missing values in the dataset. The `NA`\`s will be filled by the mean for corresponding 5-minute interval, averaged across all days.
```{r}
...
Mean total number of steps taken per day (with filled `NA`\`s) is `r mean.filled.activity` and median is `r median.filled.activity`.
These values significantly differ from the estimates from the firts part of the assignement. The mean is `r mean.filled.activity/mean.daily.activity` times bigger and median is `r median.filled.activity/median.daily.activity` times bigger.
